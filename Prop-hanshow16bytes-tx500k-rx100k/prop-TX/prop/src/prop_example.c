...
#define PROP_PAYLOAD_LEN        16+2 // A7106 256 bytes Hanshow test 16 bytes data+ 2 bytes CRC
...
#define DATA_RATE               PROP_DATA_RATE_500K // wenxue TX rate is 500K 

//uint8_t AA[4] = {0x71, 0x76, 0x41, 0x29};
//uint8_t AA[4] = {0x52, 0x56, 0x78, 0x53};
uint8_t AA[4] = {0x53, 0x78, 0x56, 0x52}; // hanshow Access Address
...
//    send_str[257] = crc_val & 0xff;
	...
	/*#define PREAMBLE_NUM            PROP_PREAMBLE_NUM_4BYTE  前导字节数  扩展的前导移到了Access Address区
		#define CRC_NUM                 PROP_CRC_NUM_NO          CRC字节数 0
		#define DATA_RATE               PROP_DATA_RATE_500K		   传输速率   */		
    ...
    {        
       ...
       // prop_mode_tx(TRX_CHANNEL, AA, PROP_AA_NUM_4BYTES, (uint8_t *)send_str, PROP_PAYLOAD_LEN);    
			...
			/* #define TRX_CHANNEL             10               决定频率
         uint8_t AA[4] = {0x53, 0x78, 0x56, 0x52};        接入地址
         #define PROP_AA_NUM_4BYTES                       接入地址长度 
         send_str                                         发送的数据
			   PROP_PAYLOAD_LEN                                 发送的数据长度
			*/	
       // prop_mode_tx2(TRX_CHANNEL, AA, PROP_AA_NUM_4BYTES, (uint8_t *)send_str, PROP_PAYLOAD_LEN); 
        prop_mode_tx5(TRX_CHANNEL, AA, PROP_AA_NUM_4BYTES, (uint8_t *)send_str, PROP_PAYLOAD_LEN);     	
