...
/**
 ****************************************************************************************
 wenxue used for test with A7106
 * @brief  Proprietary mode transmit used for expand every byte five times
 * @param[in]    ble_ch_idx     Channel frequency index
 *      Get this parameter from rf_freq_tab[](in file qnrf.c) accroding to the frequency
 *      chosen
 * @param[in]    aa_buf         Access address buffer, destination device address
 ...
 *****************************************************************************************
 */
void prop_mode_tx5(uint32_t ble_ch_idx, uint8_t *aa_buf, enum PROP_AA_NUM aa_num, uint8_t *data_buf, uint16_t data_len)
{
    uint8_t preamble_byte;
    uint8_t preamble_num;
    
	  uint8_t preamble_byte_expand[5]; // wenxue 把0x55 或者0xAA 扩展为5字节
    uint8_t aa_buf_expand[4*5]; // wenxue 把字节Access Address 扩展5倍
	  uint8_t data_buf_expand[data_len*5]; // wenxue 把Data数据扩展5倍
    uint16_t i;
   ...
   */
    /* Access Address and Payload length, Note: Payload length in bit */
  //  dp_dp_SetRegWithMask(0x00, DP_MASK_RX_PDU_LEN_IN, ((data_len+4)*2) << 3); // 前导和接入地址的扩展的字节数要在这里补上
   	dp_dp_SetRegWithMask(0x00, DP_MASK_RX_PDU_LEN_IN, ((((data_len)*5) << 3) +32)); // 前导和接入地址的扩展的字节数要在这里补上
 ...
 //   }
    ...
    for(i = 0; i < preamble_num; i++)
    {
        prop_tx_buf[5*i] = preamble_byte_expand[0];
			  prop_tx_buf[5*i+1] = preamble_byte_expand[1];	
				prop_tx_buf[5*i+2] = preamble_byte_expand[2];	
				prop_tx_buf[5*i+3] = preamble_byte_expand[3];	
				prop_tx_buf[5*i+4] = preamble_byte_expand[4];				
    }
...
//		expandbits1toN(aa_buf[3],&aa_buf_expand[6]);
		for(i = 0; i < preamble_num; i++)
    {
				expandbits1toN(aa_buf[i],&aa_buf_expand[5*i]);	  
    }
    ...
    preamble_num  = PROP_PREAMBLE_NUM;
		...
		for(i = 0; i < preamble_num; i++)
    {
			prop_tx_buf[preamble_num*5+0+5*i] = aa_buf_expand[aa_num*5-1-(5*(i+1)-1)+0]; //aa_buf_expand[15]; [10]..
			prop_tx_buf[preamble_num*5+1+5*i] = aa_buf_expand[aa_num*5-1-(5*(i+1)-1)+1]; //aa_buf_expand[16]; [11]..
			prop_tx_buf[preamble_num*5+2+5*i] = aa_buf_expand[aa_num*5-1-(5*(i+1)-1)+2]; //aa_buf_expand[17]; [12]..
			prop_tx_buf[preamble_num*5+3+5*i] = aa_buf_expand[aa_num*5-1-(5*(i+1)-1)+3]; //aa_buf_expand[18]; [13]..
			prop_tx_buf[preamble_num*5+4+5*i] = aa_buf_expand[aa_num*5-1-(5*(i+1)-1)+4]; //aa_buf_expand[19];[14]..
				  
    }
		
		
//		prop_tx_buf[preamble_num*x+0] = aa_buf_expand[aa_num*x-1-(x-1)]; //aa_buf_expand[15]; 
//	  prop_tx_buf[preamble_num*x+1] = aa_buf_expand[aa_num*x-1-(x-1)+1]; //aa_buf_expand[16]; 
//		prop_tx_buf[preamble_num*x+2] = aa_buf_expand[aa_num*x-1-(x-1)+2]; //aa_buf_expand[17]; 
//		prop_tx_buf[preamble_num*x+3] = aa_buf_expand[aa_num*x-1-(x-1)+3]; //aa_buf_expand[18]; 
//		prop_tx_buf[preamble_num*x+4] = aa_buf_expand[aa_num*x-1-(x-1)+4]; //aa_buf_expand[19]; 
//		
//		
//		prop_tx_buf[preamble_num*x+0+5*1] = aa_buf_expand[aa_num*x-1-(2*x-1)]; //aa_buf_expand[10]; 
//	  prop_tx_buf[preamble_num*x+1+5*1] = aa_buf_expand[aa_num*x-1-(2*x-1)+1]; //aa_buf_expand[11]; 
//		prop_tx_buf[preamble_num*x+2+5*1] = aa_buf_expand[aa_num*x-1-(2*x-1)+2]; //aa_buf_expand[12]; 
//		prop_tx_buf[preamble_num*x+3+5*1] = aa_buf_expand[aa_num*x-1-(2*x-1)+3]; //aa_buf_expand[13]; 
//		prop_tx_buf[preamble_num*x+4+5*1] = aa_buf_expand[aa_num*x-1-(2*x-1)+4]; //aa_buf_expand[14];
//		
//		
//		prop_tx_buf[preamble_num*x+0+5*2] = aa_buf_expand[aa_num*x-1-(3*x-1)]; //aa_buf_expand[5]; 
//	  prop_tx_buf[preamble_num*x+1+5*2] = aa_buf_expand[aa_num*x-1-(3*x-1)+1]; //aa_buf_expand[6]; 
//		prop_tx_buf[preamble_num*x+2+5*2] = aa_buf_expand[aa_num*x-1-(3*x-1)+2]; //aa_buf_expand[7]; 
//		prop_tx_buf[preamble_num*x+3+5*2] = aa_buf_expand[aa_num*x-1-(3*x-1)+3]; //aa_buf_expand[8]; 
//		prop_tx_buf[preamble_num*x+4+5*2] = aa_buf_expand[aa_num*x-1-(3*x-1)+4]; //aa_buf_expand[9];
//		
//		prop_tx_buf[preamble_num*x+0+5*3] = aa_buf_expand[aa_num*x-1-(4*x-1)]; //aa_buf_expand[0]; 
//	  prop_tx_buf[preamble_num*x+1+5*3] = aa_buf_expand[aa_num*x-1-(4*x-1)+1]; //aa_buf_expand[1]; 
//		prop_tx_buf[preamble_num*x+2+5*3] = aa_buf_expand[aa_num*x-1-(4*x-1)+2]; //aa_buf_expand[2]; 
//		prop_tx_buf[preamble_num*x+3+5*3] = aa_buf_expand[aa_num*x-1-(4*x-1)+3]; //aa_buf_expand[3]; 
//		prop_tx_buf[preamble_num*x+4+5*3] = aa_buf_expand[aa_num*x-1-(4*x-1)+4]; //aa_buf_expand[4];
...
//    }
	  ...
	  for(i = 0; i < data_len; i++)
    {
		  	expandbits1toN(data_buf[i],&data_buf_expand[5*i]);
    }
    ...
    {
        prop_tx_buf[preamble_num*5 + aa_num*5 + i] = data_buf_expand[i];
    }
